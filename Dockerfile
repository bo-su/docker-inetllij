...
MAINTAINER Florin Patan "bosu-cn@gmail.com"
...
RUN echo 'Downloading Go 1.8.1' && \
    wget https://storage.googleapis.com/golang/go1.8.1.linux-amd64.tar.gz -O /tmp/go.tar.gz -q && \
    echo 'Installing Go 1.8.1' && \
    sudo tar -zxf /tmp/go.tar.gz -C /usr/local/ && \
    rm -f /tmp/go.tar.gz

RUN echo 'Installing Go plugin' && \
    wget https://plugins.jetbrains.com/files/5047/33849/Go-0.13.1947.zip -O /home/developer/.IdeaIC2017.1/config/plugins/go.zip -q && \
    cd /home/developer/.IdeaIC2017.1/config/plugins/ && \
    unzip -q go.zip && \
    rm go.zip

RUN echo 'Installing Markdown plugin' && \
    wget https://plugins.jetbrains.com/files/7793/34427/markdown-2017.1.20170419.zip -O markdown.zip -q && \
